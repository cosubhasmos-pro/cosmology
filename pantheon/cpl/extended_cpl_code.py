...
def wd(z, params):
    od0, H0, w0, wa, w2 = params
    return w0 + wa * z / (1 + z) + w2*(z/(1+z))**2 
...
if __name__ == '__main__':
    ...
    # Initial positions
    p0 = np.random.normal(loc=[0.7, 70, -1.0, 0.0,-2],
                         scale=[0.05, 5, 0.2, 0.5,0.2],
                         size=(nwalker, ndim))

    # Run MCMC
    ncpu = max(15, cpu_count())
    ...
    fig = corner.corner(
        samples,
        labels=[r'$\Omega_d$', r'$H_0$', r'$w_0$', r'$w_a$',r'$w_2$'],
        quantiles=[0.16, 0.5, 0.84],
        show_titles=True,
        title_kwargs={"fontsize": 12}
    )
    fig.savefig('SN_CPL_extended_corner.pdf')
    plt.close()
